...
export function LagerCalc() {
  ...
  const items: { label: ReactNode; value: string }[] = result
    ? ([
        {
          label: (
            <>
        ...
        stil
          ? {
              label: (
                <>
                  {tx(locale, "As", "Shaft")} <span className="normal-case">h6</span>{" "}
          ...
          <Field label={tx(locale, "Rotatie", "Rotation")}>
            <SelectInput value={rot} onChange={setRot}>
              <option value="binnen">
                {tx(
                  locale,
                  "Omlopende last op binnenring (meestal: as draait)",
                  "Circumferential load on inner ring (usually: shaft rotates)",
                )}
              </option>
              <option value="buiten">
                {tx(
                  locale,
                  "Omlopende last op buitenring (meestal: naaf draait)",
                  "Circumferential load on outer ring (usually: hub rotates)",
                )}
              </option>
              <option value="stil">
                {tx(
                  locale,
                  "Puntbelasting op binnenring, verschuifbaar",
                  "Point load on inner ring, axially free",
                )}
              </option>
            </SelectInput>
          </Field>
          ...
          <table className="ref-table">
            ...
            <tbody>
              <tr className={rot === "binnen" && load === "licht" ? "is-active" : ""}>
                <th scope="row">
                  {tx(locale, "Omlopende last binnenring", "Circumferential load, inner")}
                </th>
              ...
              <tr className={rot === "buiten" && load === "licht" ? "is-active" : ""}>
                <th scope="row">
                  {tx(locale, "Omlopende last buitenring", "Circumferential load, outer")}
                </th>
                <td>{tx(locale, "Licht, P ≤ 0,05 C", "Light, P ≤ 0.05 C")}</td>
                <td>g6</td>
                <td>M7</td>
