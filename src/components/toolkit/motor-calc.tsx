...
export function MotorCalc() {
  ...
  function onUnit(next: string) {
    ...
    if (n != null && n > 0 && unit !== speedUnit) {
      setSpeed(unit === "m/s" ? fmtInput(n / 60) : fmtInput(n * 60));
    }
    setSpeedUnit(unit);
  }
  ...
  const result =
    v_ms != null && D_m != null && mRaw != null && muRaw != null && etaRaw != null && fbRaw != null
      ? sizeMotor({
          v_ms,
          D_m,
          mass_kg: mRaw,
          ...
          <Field label={tx(locale, "Bedrijf", "Duty")}>
            <SelectInput value={duty} onChange={onDuty}>
              <option value="rollenbaan">
                {tx(locale, "Rollenbaan (μ = 0,03)", "Roller conveyor (μ = 0.03)")}
              </option>
            ...
            <ResultGrid
              items={[
                ...
                },
                {
                  ...
                  value: result.i == null ? "—" : `≈ ${fmtDotComma(result.i, 1)}`,
                },
                {
      ...
      <section className="mt-10">
        ...
        <div className="table-scroll mt-4">
          <table className="ref-table">
            ...
            <tbody>
              {IEC_KW.map((kw) => (
                <tr key={kw} className={kw === result?.iecKw ? "is-active" : ""}>
