      ...
      <CalcPanel>
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          ...
          <Field label={tx(locale, "Inbouw", "Installation")}>
            <SelectInput value={kind} onChange={(v) => setKind(v as SeegerKind)}>
        ...
        <SchemaPanel
          caption={
            kind === "as"
              ? tx(
                  locale,
                  "As · DIN 471 — groef op de buitenkant",
                  "Shaft · DIN 471 — groove on the outside",
                )
              : tx(
                  locale,
                  "Boring · DIN 472 — groef op de binnenkant",
                  "Bore · DIN 472 — groove on the inside",
                )
          }
        >
          <CirclipSection kind={kind} d1={row?.d1} d2={result?.d2} b={result?.b} t={result?.t} />
        </SchemaPanel>
      </section>
