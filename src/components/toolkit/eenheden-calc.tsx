...
import { CalcEyebrow, CalcPanel, CopyLink, CopyResult, Field, Note, SelectInput } from "./calc-ui";
...
export function EenhedenCalc() {
  ...
  const fromShown = edited === "to" && toTyped != null ? convert(toTyped, to, from) : fromValue;
  const toShown = edited === "from" && fromValue != null ? convert(fromValue, from, to) : toTyped;

  const rawFromDisplay =
    edited === "from" ? rawFrom : fromShown == null ? "" : formatQty(fromShown);
  const rawToDisplay = edited === "to" ? rawToEdit : toShown == null ? "" : formatQty(toShown);
      ...
      <CalcPanel>
        ...
        {fromShown != null && toShown != null ? (
          ...
          <p className="mt-5 text-sm text-muted">
            {tx(locale, "Voer een getal in.", "Enter a number.")}
          </p>
        )}
      </CalcPanel>
      ...
      <section className="mt-12">
        ...
        <div className="table-scroll mt-4">
          <table className="ref-table">
            ...
            <tbody>
              {category.units.map((unit) => {
                const shown = sourceValue == null ? null : convert(sourceValue, sourceUnit, unit);
