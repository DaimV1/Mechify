...
export function CylinderCalc() {
  ...
  const liters = pick && p != null && strokeMm != null ? cycleLiters(pick, p, strokeMm) : null;
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          ...
          <Field label={tx(locale, "Richting", "Direction")}>
            <SelectInput value={dir} onChange={(v) => setDir(v as StrokeDir)}>
              ...
              <option value="in">
                {tx(locale, "Binnenhalen (stangzijde)", "Retract (rod side)")}
              </option>
            </SelectInput>
          </Field>
        ...
        {pick && forces && need != null && dMin != null && p != null ? (
          <>
            ...
            <ResultGrid
              items={[
                {
                  label: tx(locale, "Boring / stang", "Bore / rod"),
                  value: `Ø${pick.bore} / ${pick.rod} mm`,
                },
          ...
          <p className="mt-5 text-sm text-muted">
            {tx(
              locale,
              "Vul last, druk en lastfactor in.",
              "Enter load, pressure and load factor.",
            )}
          </p>
        )}
      </CalcPanel>
